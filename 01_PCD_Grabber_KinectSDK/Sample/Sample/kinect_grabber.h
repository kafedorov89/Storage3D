...
namespace pcl
{
	...
	pcl::KinectGrabber::KinectGrabber( const int index )
		...
		, quit( false )
	{
		RGBMode = true; //Switch on RGBXYZ mode
	...
	void pcl::KinectGrabber::start()
	{
		...
		//thread = boost::thread(&KinectGrabber::getCloudInThread, this);
		getCloudInThread();
	}
	...
	void pcl::KinectGrabber::getCloudInThread()
	{
		boost::unique_lock<boost::mutex> lock( mutex );
	...
	void pcl::KinectGrabber::convertDepthToPointXYZ(NUI_LOCKED_RECT* depthLockedRect)
	{
	...
	void pcl::KinectGrabber::convertRGBDepthToPointXYZRGB(NUI_LOCKED_RECT* colorLockedRect, NUI_LOCKED_RECT* depthLockedRect)
	{
		NUI_DEPTH_IMAGE_PIXEL* depthPixel = reinterpret_cast<NUI_DEPTH_IMAGE_PIXEL*>( depthLockedRect->pBits );
