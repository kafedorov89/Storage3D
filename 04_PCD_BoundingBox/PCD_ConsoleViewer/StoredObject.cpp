...
StoredObject::StoredObject(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, float limit_array[6], int stepdegree, int maxdegree, float griddensity)
{
	width = FLT_MAX;
	lenght = FLT_MAX;
	height = FLT_MAX;
...
void StoredObject::find_bbox(){
	...
	//vg.setLeafSize(cur_x_width / 10.0f, cur_y_lenght / 10.0f, cur_z_height / 10.0f);
	//vg.setLeafSize(cur_x_width / 10.0f, cur_x_width / 10.0f, cur_x_width / 10.0f);
	...
	//pcl::transformPointCloud(*zero_point_cloud, *minimal_cloud, jump_to_zero);
